...
class RfbProto {
  ...
  // length is 4 modifier down events, two key events followed by 4
  ...
  void writeKeyEvent(KeyEvent evt) throws IOException {
    ...
    if (viewer.options.highLatencyKeys) {
      if (down) {
	// Repeat is turned off, presumably for high latency links.
	// Immediately release a key after it has been pressed.
	down = false;
	writeKeyEvent(key, down);
      }
      else {
	// Do not send up events, they are sent after down events.
	eventBufLen = 0;
      }
    }
